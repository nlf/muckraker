...
class QueryBuilder {
  ...
  getColumnNames (columns) {
    ...
    return columns.filter((column) => {
      ...
      if (Array.isArray(column)) {
        const name = column[0]
        return Object.prototype.hasOwnProperty.call(this._columns, name) && ['json', 'jsonb'].includes(this._columns[name].type)
      }

      // we omit encrypted columns unless the user specifically asks for them
      return Object.prototype.hasOwnProperty.call(this._columns, column) && (userDefined || !this._columns[column].encrypted)
    }).map((column) => {
      if (Array.isArray(column)) {
  ...
  generateWhere (input = {}, prefix, force) {
    ...
    if (!force && Object.prototype.hasOwnProperty.call(this._columns, this._timestamps.deleted) && !Object.prototype.hasOwnProperty.call(input, this._timestamps.deleted)) {
      input[this._timestamps.deleted] = null
    }

    for (const column in input) {
      // skip properties that don't reflect a real column
      if (!Object.prototype.hasOwnProperty.call(this._columns, column)) {
        continue
      }
  ...
  parseInput (input, prefix, update) {
    ...
    if (!update && Object.prototype.hasOwnProperty.call(this._columns, this._timestamps.created) && !Object.prototype.hasOwnProperty.call(params, createdName)) {
      columns.push(`"${this._timestamps.created}"`)
      params[createdName] = now
      data.push(`$[${createdName}]`)
    }

    if (Object.prototype.hasOwnProperty.call(this._columns, this._timestamps.updated) && !Object.prototype.hasOwnProperty.call(params, updatedName)) {
