...
class Table {
  constructor(db, name, columns, cipher) {

    this._db = db;
    this._name = name;
    this._columns = columns;
    this._cipher = cipher;
    this._columnList = Object.keys(this._columns).map((column) => {

      if (!this._columns[column].encrypted) {
        return `"${column}"`;
      }

      return `pgp_sym_decrypt("${column}",'${this._columns[column].encryptionKey}','cipher-algo=${this._cipher}')`;
    });
  }
  ...
  find(when) {
    ...
    let query = `SELECT ${this._columnList} FROM $[_table~]`;
  ...
  insert(data) {
    ...
    const query = `INSERT INTO $[_table~] (${formatted.columns.join(',')}) VALUES (${formatted.data.join(',')}) RETURNING ${this._columnList}`;
    return this._db.oneOrNone(query, formatted.params);
  }
  ...
  update(when, data) {
    ...
    query += ` RETURNING ${this._columnList}`;
