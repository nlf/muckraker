...
exports.load = function (db, root) {
  ...
  const validate = function (scriptPath, script) {
    ...
    script.returns = script.returns.split('||').map(mask => mask.trim()).reduce((final, mask) => {
      assert(Object.prototype.hasOwnProperty.call(PG.queryResult, mask), `Script at ${path} specified invalid return type "${mask}`)
      return final + PG.queryResult[mask]
    }, 0)
    ...
    if (script.transaction) {
      ...
      if (script.transaction.isolation) {
        assert(Object.prototype.hasOwnProperty.call(PG.txMode.isolationLevel, script.transaction.isolation), `Script at ${path} specified invalid transaction isolation level "${script.transaction.isolation}"`)
      }
      script.transaction.mode = new PG.txMode.TransactionMode({
