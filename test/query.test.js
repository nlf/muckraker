...
describe('#find()', () => {
  test('defaults to returning all columns', async () => {
    ...
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users"`)
  })
  ...
  test('allows ignoring default NOT NULL clause on "deleted_at"', async () => {
    ...
    expect(query).toEqual(`SELECT ${db.entries._formatColumns()} FROM "entries" WHERE "deleted_at" IS NOT NULL`)
  })
  ...
  test('can find rows in a table with a column that is not null', async () =>  {
    ...
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "unknown" IS NOT NULL`)
  })

  test('can find rows in a table with a json value that is not null', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ blob: { some: { $ne: null } } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "blob"#>>'{some}' IS NOT NULL`)
  })

  test('can use the $gt operator', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ pets: { $gt: 1 } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "pets" > 1`)
  })

  test('can use the $gt operator on a json column', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ blob: { some: { value: { $gt: 5 } } } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "blob"#>>'{some,value}' > 5`)
  })
  ...
  test('can use the $in operator', async () =>  {
    ...
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "pets" IN (1,2,3)`)
  })

  test('can use the $in operator on a json column', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ blob: { some: { value: { $in: [1, 2, 3] } } } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "blob"#>>'{some,value}' IN (1,2,3)`)
  })

  test('can use the $nin operator', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ pets: { $nin: [1, 3] } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "pets" NOT IN (1,3)`)
  })

  test('can use the $nin operator on a json column', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ blob: { some: { value: { $nin: [1, 2, 3] } } } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "blob"#>>'{some,value}' NOT IN (1,2,3)`)
  })

  test('can use the $like operator', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ user_name: { $like: 'test' } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "user_name" LIKE 'test'`)
  })

  test('can use the $like operator on a json column', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ blob: { some: { value: { $like: 'test' } } } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "blob"#>>'{some,value}' LIKE 'test'`)
  })

  test('can use the $nlike operator', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ user_name: { $nlike: 'test' } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "user_name" NOT LIKE 'test'`)
  })

  test('can use the $nlike operator on a json column', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.find({ blob: { some: { value: { $nlike: 'test' } } } })
    expect(query).toEqual(`SELECT ${db.users._formatColumns()} FROM "users" WHERE "blob"#>>'{some,value}' NOT LIKE 'test'`)
  })
})
