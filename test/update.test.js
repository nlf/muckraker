...
describe('#update()', () => {
  test('can update a row', async () =>  {
    ...
    expect(query).toEqual(`UPDATE "users" SET "user_name" = 'test_user', "blob" = '{"another":"thing"}' WHERE "id" = 0 AND "blob"#>>'{some}' = 'thing' RETURNING ${db.users._formatColumns()}`)
  })

  test('can update a row without a query', async () =>  {
    const db = new Muckraker(getOptions())
    const query = await db.users.update(null, { user_name: 'test_user' })
    expect(query).toEqual(`UPDATE "users" SET "user_name" = 'test_user' RETURNING ${db.users._formatColumns()}`)
  })
...
describe('#updateOne()', () => {
  test('can update a row', async () =>  {
    ...
    expect(query).toEqual(`UPDATE "users" SET "user_name" = 'test_user' WHERE "id" = 0 RETURNING ${db.users._formatColumns()}`)
  })
