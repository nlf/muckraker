...
test('constructor', async () => {
  ...
  test('correctly rejects with an error when connection fails', async (t) => {
    t.rejects(() => {
      const db = new Muckraker({ _mocked: new Mock(true) })
      return db.advisories.find()
    }, 'Failed to connect')
  })
})
...
test('find', async () =>  {
  ...
  test('ignores invalid json properties', async (t) =>  {
    const db = new Muckraker(internals)
    const query = await db.users.find({}, [['id', 'invalid', 'path'], 'user_name', ['invalid', 'path'], ['json_blob', 'real', 'path']])
